...
禅道提单时 `project` 字段**默认填「基建S」（project: 5）**，不要留空，除非另行指定。

## 严重度与优先级

**禁止无脑填同一档**，必须按实际影响判断：

| 档位 | severity（严重度） | pri（优先级） |
|------|-------------------|--------------|
| 1 | 核心功能崩溃 / 数据丢失 / 安全漏洞 | 影响发版 / 线上故障 / 高频路径 |
| 2 | 主流程无法完成，无绕行方案 | 本迭代需修，不阻塞发版 |
| 3 | 功能异常但有绕行，或影响范围有限 | 可排期修，不影响里程碑 |
| 4 | UI 瑕疵、文案错误、非核心体验问题 | 优化项，有时间再做 |
