...
```bash
python3 generate.py --prompt "a cat walking in a garden" --aspect-ratio "16:9"
```

Options:
- `--prompt` (required): video description
- `--aspect-ratio`: `16:9` (default), `9:16`, `1:1`
- `--image-urls URL1 URL2 ...`: reference images for image-to-video (max 4)
- `--out`: output filename (default: `petclaw-<timestamp>.mp4`)
...
OUTPUT_DIR = os.path.expanduser("~/.openclaw/workspace/outputs/petclaw-video-gen")
...
def _download(url: str, out_file: str, timeout_s: int = 300):
    req = urllib.request.Request(url, method="GET", headers={"User-Agent": _UA})
    try:
        with urllib.request.urlopen(req, timeout=timeout_s) as resp:
            content = resp.read()
    except Exception as e:
        raise RuntimeError(f"Failed to download video: {e}") from None
    with open(out_file, "wb") as f:
        f.write(content)


def _default_out_file(ext: str = ".mp4") -> str:
...
def _ext_from_url(url: str) -> str:
    ...
    return ext.lower() if ext in (".mp4", ".mov", ".webm") else ".mp4"


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Generate a video via PetClaw API.")
    parser.add_argument("--prompt", required=True, help="Video description (1–5000 chars).")
    parser.add_argument("--aspect-ratio", default="16:9",
                        choices=["16:9", "9:16", "1:1"],
                        help="Aspect ratio: 16:9 (default), 9:16, 1:1.")
    parser.add_argument("--image-urls", nargs="*", default=None,
                        help="Reference image URLs for image-to-video (max 4).")
    parser.add_argument("--out", default=None, help="Output filename (default: petclaw-<timestamp>.mp4).")
    parser.add_argument("--poll-seconds", type=int, default=15, help="Seconds between polls.")
    parser.add_argument("--max-retries", type=int, default=20, help="Max polling attempts (~5 min).")
    ...
    if args.image_urls and len(args.image_urls) > 4:
        print("Error: maximum 4 reference image URLs allowed for image-to-video.", file=sys.stderr)
        return 2
    ...
    for attempt in range(2):
        resp = _json_request(f"{API_BASE}/video/generations", api_key=api_key, method="POST", payload=payload)
        code = resp.get("code")
        if code == 200:
            break
    ...
    for i in range(1, args.max_retries + 1):
        ...
        poll = _json_request(f"{API_BASE}/tasks/{task_id}?type=video", api_key=api_key, method="GET")
    ...
    raise RuntimeError(
        f"Timed out after {args.max_retries} polls "
        f"(~{args.max_retries * args.poll_seconds // 60} min). Task ID: {task_id}"
    )
