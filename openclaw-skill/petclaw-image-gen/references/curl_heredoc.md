...
Replace:
- `<USER_PROMPT>` with the image description
- `<ASPECT_RATIO>` with `1:1`, `16:9`, `9:16`, `4:3`, or `3:4`
...
if [ -z "$OUT_FILE" ]; then
  OUT_FILE="petclaw-$(date +%Y%m%d-%H%M%S).png"
fi

OUTPUT_DIR="$HOME/.openclaw/workspace/outputs/petclaw-image-gen"
mkdir -p "$OUTPUT_DIR"
...
_submit() {
  curl -s -X POST "https://petclaw.ai/api/v1/image/generations" \
...
# Step 2: Poll (every 10s, up to 30 retries = ~5 min)
MAX_RETRIES=30
for i in $(seq 1 $MAX_RETRIES); do
  sleep 10
  TASK=$(curl -s "https://petclaw.ai/api/v1/tasks/${TASK_ID}?type=image" \
    -H "Authorization: Bearer $API_KEY")
  STATE=$(echo "$TASK" | python3 -c "import sys,json; print(json.load(sys.stdin).get('data',{}).get('state',''))" 2>/dev/null)

  if [ "$STATE" = "success" ]; then
    IMAGE_URL=$(echo "$TASK" | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['generate_urls'][0]['url'])" 2>/dev/null)
...
Same flow, but add `image_input` to the JSON body (max 14 URLs):

```bash
API_KEY=$(cat ~/.openclaw/workspace/skills/petclaw-image-gen/.petclaw.key 2>/dev/null)
# ... (same sanitization and escape setup as above) ...

RESP=$(curl -s -X POST "https://petclaw.ai/api/v1/image/generations" \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d @- <<PETCLAW_END
{"prompt":"$PROMPT_ESC","aspect_ratio":"$ASPECT_RATIO","image_input":["<URL1>","<URL2>"]}
PETCLAW_END
)
